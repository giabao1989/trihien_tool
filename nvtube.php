...
////// phan function nv job youtube
function jobtup () {global $tsm, $url, $idyou;
$url = "https://gateway.golike.net/api/advertising/publishers/youtube/jobs?account_id=$idyou";
$goi = get_url($url, $tsm)['data'];
$erro = $goi['message'];
    if (str_contains($erro, 'Không tồn tại tài khoản làm việc hoặc tài khoản bị khóa !')) {return "KHONG TON TAI";}
elseif (str_contains($erro, 'Bạn đã làm quá số jobs quy định của hệ thống đối với 1 tài khoản')) {return "DA GIOI HAN";}
elseif (str_contains($erro, 'Hiện tại chưa có jobs mới')) {return "HET JOB";}
$chek = $goi['status'];
if ($chek != 200) return "HET JOB";
/////////////////////////////////////////////
/////////////////////////////////////////////
$id    = $goi['lock']['ads_id'];
$video = $goi['lock']['object_id'];
$type  = $goi['lock']['type'];
$dabao = ["ads_id" => $id,"object_id" => $video,"account_id" => $idyou,"type" => $type];
/////////////////////////////////////////////
/////////////////////////////////////////////
if ($type == "subscribe") {
$danhan = ["ads_id" => $id,"account_id" => $idyou,"async" => true];
$link = $goi['data']['link'];
return ['video' => $video, 'link' => $link, 'danhan' => $danhan, 'ketqua' => 'DANG KY', 'dabao' => $dabao, 'idlam' => $id];}
/////////////////////////////////////////////
/////////////////////////////////////////////
elseif ($type == "comment") {
$chanid   = $goi['lock']['comment_id'];
$binhluan = $goi['lock']['message'];
$danhan = ["ads_id" => $id,"account_id" => $idyou,"async" => true,"data" => null,"comment_id" => $chanid,"message" => $binhluan];
return ['video' => $video, 'danhan' => $danhan, 'ketqua' => 'COMMEN', 'dabao' => $dabao, 'binhluan' => $binhluan, 'idlam' => $id];}
}


function nhantup() {global $tsm, $url, $data, $datanhan;
$url = "https://gateway.golike.net/api/advertising/publishers/youtube/complete-jobs";
$data = $datanhan;


$goi  = post_url($url, $tsm, $data)['data'];
if (!$goi) return "THAT BAI";

//print_r($goi);
$chek = $goi['status'];
if ($chek != 200) return "THAT BAI";

$tien = $goi['data']['prices'];
return $tien;}
...
function baotup () {global $tsm, $url, $data, $databao;
...
function goitc() {global $thanhcong, $accounts, $vitri, $tienxu, $baodi, $caigiday;
...
$mui = 5; goiecho();$mui = '';sleep(3);mothai();
...
$bai++;nhantb();continue;}
$mui = 8; goiecho();$mui = '';sleep(2);mothai();
$thanhcong++;
goitc();sleep(3);
